...
# Append a terminator to the message
msg += chr(0)  # Use null character as terminator
...
for i in range(len(msg)):
    ...
    new_value = (int(img[n, m, z]) + ord(msg[i]) + hashed_password[i % len(hashed_password)]) % 256
    img[n, m, z] = new_value
...
# Open the newly saved encrypted image (macOS compatible)
subprocess.call(['open', encrypted_image_path])
